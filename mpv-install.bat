...
:: Register VapourSynth location in user PATH variable
set "PathToAdd=%cd%\portable_config\VapourSynth"

for /f "tokens=2*" %%A in ('reg query HKCU\Environment /v Path 2^>nul') do set "CurrentPath=%%B"

echo %CurrentPath% | find /i "%PathToAdd%" >nul
if errorlevel 1 (
    setx Path "%CurrentPath%;%PathToAdd%"
)
...
call :reg add "%app_key%" /v "FriendlyAppName" /d "mpv-synth" /f
...
set capabilities_key=HKLM\SOFTWARE\Clients\Media\mpv-synth\Capabilities
call :reg add "%capabilities_key%" /v "ApplicationName" /d "mpv-synth" /f
call :reg add "%capabilities_key%" /v "ApplicationDescription" /d "mpv-synth media player" /f
...
call :reg add "HKLM\SOFTWARE\RegisteredApplications" /v "mpv-synth" /d "SOFTWARE\Clients\Media\mpv-synth\Capabilities" /f
...
if %errorlevel% equ 0 (
    pwsh -NoProfile -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%ProgramData%\Microsoft\Windows\Start Menu\Programs\mpv-synth.lnk');$s.TargetPath='%mpv_path%';$s.Save()"
) else (
    powershell -NoProfile -Command "$s=(New-Object -COM WScript.Shell).CreateShortcut('%ProgramData%\Microsoft\Windows\Start Menu\Programs\mpv-synth.lnk');$s.TargetPath='%mpv_path%';$s.Save()"
)
echo.
echo Installed successfully^^! You can now configure mpv-synth's file associations in the
