...
:: Remove VapourSynth path from user PATH variable
cd /D %~dp0\..
...
if not errorlevel 1 (
    :: Remove the entry - handle both ;path and path; forms
...
reg delete "HKLM\SOFTWARE\RegisteredApplications" /v "mpv-synth" /f >nul
reg delete "HKLM\SOFTWARE\Clients\Media\mpv-synth" /f >nul
...
del "%ProgramData%\Microsoft\Windows\Start Menu\Programs\mpv-synth.lnk"
