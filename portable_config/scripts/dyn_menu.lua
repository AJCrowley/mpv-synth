...
-- https://github.com/tsl0922/mpv-menu-plugin/blob/main/src/lua/dyn_menu.lua
...
if use_mpv_impl then
    ...
    mp.observe_property('current-vo', 'native', function(name, val)
        if val then
            -- Commit immediately rather than waiting for the next idle tick,
            -- otherwise right-clicking before the tick fires shows the default menu.
            mp.set_property_native(menu_prop, menu_items)
            menu_items_dirty = false
        end
    end)

    mp.add_forced_key_binding('MBTN_RIGHT', 'show-context-menu', function()
        -- Always commit menu data immediately before showing the menu.
        -- This eliminates any race condition between our observers and the click.
        mp.set_property_native(menu_prop, menu_items)
...
if conf then
    ...
    load_dyn_menus()
    -- The VO may not be ready when this script first loads, so menu-data writes
    -- made immediately can be silently lost. Observe vo-configured and commit
    -- as soon as the VO signals it is ready, then unregister so it only fires once.
    local function commit_menu()
        mp.set_property_native(menu_prop, menu_items)
    end

    -- Watch menu-data itself -- if mpv clears it during VO initialisation,
    -- resubmit immediately. This catches the case where our earlier commits
    -- are wiped by mpv's own initialisation cycle.
    mp.observe_property(menu_prop, 'native', function(name, val)
        if (val == nil or (type(val) == 'table' and #val == 0)) and #menu_items > 0 then
            msg.debug('menu-data was cleared, resubmitting')
            commit_menu()
        end
    end)

    -- Also commit on file-loaded for subsequent files in the same session
    mp.register_event('file-loaded', function()
        commit_menu()
    end)

    commit_menu()  -- attempt immediate commit in case window is already up
end
